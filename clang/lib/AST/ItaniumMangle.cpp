...
void CXXNameMangler::mangleReflection(const APValue &R) {
  ...
  case ReflectionKind::Template: {
    ...
    // llvm_unreachable in mangleUnqualifiedName (TC-0008). members_of over a
    ...
    // definitions and a single body serves both call sites (TC-0004; the
    // AST-level specializations are correct and distinct). Append a structural
    ...
    if (auto *FTD = dyn_cast<FunctionTemplateDecl>(TD)) {
      ...
      // overloads (const&/&/const&&/&&, identical heads) all folded
      // (TC-0009). Hash the pattern's function type (return type, parameter
